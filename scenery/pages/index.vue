...
function syncSkySunLightFromSkyboxSettings(settings: SceneSkyboxSettings): void {
  ...
  light.intensity = clampNumber(exposure, 0, 20, resolveSceneExposure(DEFAULT_SKYBOX_SETTINGS.exposure));
}
...
async function handleLoadSceneEvent(event: Extract<BehaviorRuntimeEvent, { type: 'load-scene' }>): Promise<void> {
  ...
  uni.navigateTo({ url: `/uni_modules/scene-viewer/pages/index?id=${encodeURIComponent(sceneId)}` });
}
