...
import { Button, Form, Input, message, Modal, Space, Tag } from 'ant-design-vue'
...
const formModel = reactive<SeriesFormModel>({
  name: '',
  description: '',
})
...
async function submit() {
  ...
  try {
    if (editingId.value) {
      await updateResourceSeriesApi(editingId.value, {
        name: formModel.name.trim(),
        description: formModel.description || null,
      })
      message.success('系列已更新')
    } else {
      await createResourceSeriesApi({ name: formModel.name.trim(), description: formModel.description || undefined })
...
const [Grid, gridApi] = useVbenVxeGrid<any>({
  gridOptions: {
    ...
    columns: [
      ...
      { field: 'assetCount', minWidth: 120, title: '资源数', slots: { default: 'assetCount' } },
    ...
    proxyConfig: {
      ajax: {
        query: async () => {
          const list = await listResourceSeriesApi()
  ...
  <div class="p-5">
    <Grid>
      <template #toolbar-actions>
        <Button type="primary" @click="openCreate">新增系列</Button>
      </template>

      <template #assetCount="{ row }">
        <Tag v-if="row.assetCount" color="blue">{{ row.assetCount }}</Tag>
