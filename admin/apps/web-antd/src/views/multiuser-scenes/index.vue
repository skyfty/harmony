...
function resolveMultiuserSummary(row: SceneItem) {
  ...
  return {
    ...
    replicatedNodeCount: Number(multiuser?.replicatedNodeCount ?? 0) || 0,
    replicatedPhysicsNodeCount: Number(multiuser?.replicatedPhysicsNodeCount ?? 0) || 0,
    physicsAuthorityNodeCount: Number(multiuser?.physicsAuthorityNodeCount ?? 0) || 0,
    scenes,
    primaryScene,
...
const [MultiuserGrid] = useVbenVxeGrid<MultiuserRow>({
  ...
  gridOptions: {
    ...
    columns: [
      ...
      { field: 'multiuserSummary.replicatedNodeCount', minWidth: 130, title: '同步节点' },
      { field: 'multiuserSummary.replicatedPhysicsNodeCount', minWidth: 130, title: '物理同步节点' },
