...
import { listFeaturedSpotsApi, createFeaturedSpotApi, updateFeaturedSpotApi, deleteFeaturedSpotApi } from '#/api/core/hotFeatured';
import { Button, Modal, message, Form, InputNumber, Select } from 'ant-design-vue';
...
const [Grid, gridApi] = useVbenVxeGrid<HotFeaturedItem>({
  formOptions: {
    schema: [
      { component: 'Select', fieldName: 'sceneSpotId', label: '景点', componentProps: { options: sceneOptions, loading: sceneOptionsLoading, showSearch: true, filterOption: false, onSearch: (v: string) => handleSceneSearch(v) } },
      { component: 'InputNumber', fieldName: 'order', label: '排序', componentProps: { style: { width: '120px' } } },
    ],
  },
  gridOptions: {
    border: true,
    ...
    proxyConfig: { ajax: { query: async () => { 
        const res = await listFeaturedSpotsApi(); 
...
async function handleAdd(values: any) {
  try {
    await createFeaturedSpotApi({ sceneSpotId: values.sceneSpotId, order: values.order || 0 });
...
async function handleUpdateOrder(row: any) {
  try {
    await updateFeaturedSpotApi(row.id, { order: row.order });
...
async function submitForm() {
  ...
  try {
    if (editingId.value) {
      await updateFeaturedSpotApi(editingId.value, { order: formModel.value.order });
      message.success('更新成功');
    } else {
      await createFeaturedSpotApi({ sceneSpotId: formModel.value.sceneSpotId, order: formModel.value.order });
      message.success('添加成功');
    }
  ...
  <div class="p-5">
    <Grid>
      <template #toolbar-actions>
        <Button type="primary" @click="openCreateModal">添加精选景点</Button>
      </template>
