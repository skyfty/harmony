...
function pickGradient(index: number): string {
  const normalized = Number.isFinite(index) ? index : Math.floor(Math.random() * gradientPalette.length)
...
function mapWorkSummary(summary: WorkSummary, index: number): WorkItem {
  ...
  return {
    ...
    duration: metadata && typeof metadata === 'object' && 'duration' in metadata
...
export const useWorksStore = defineStore('worksStore', {
  ...
  actions: {
    ...
    async createCollection(payload: { title: string; description?: string; coverUrl?: string; workIds?: string[]; isPublic?: boolean }) {
      try {
    ...
    async updateCollection(id: string, payload: Parameters<typeof apiUpdateCollection>[1]) {
      try {
        ...
        this.collections = this.collections.map((collection) => (collection.id === id ? mapped : collection))
        if (payload.workIds) {
    ...
    async addWorksToCollection(workIds: string[], collectionId: string) {
      ...
      try {
        ...
        const mapped = mapCollectionSummary(updated, this.collections.length)
        this.collections = this.collections.map((collection) => (collection.id === collectionId ? mapped : collection))
    ...
    async withdrawExhibition(id: string) {
      try {
        ...
        this.exhibitions = this.exhibitions.map((item) => (item.id === id ? mapped : item))
      } catch (error) {
    ...
    async purchaseProduct(id: string, payload: PurchasePayload) {
      try {
        ...
        this.products = this.products.map((item) => (item.id === id ? updatedProduct : item))
