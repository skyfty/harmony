...
export const SKIN_PRODUCT_CATEGORY_NAME = '皮肤'
...
export async function listProductCategories(options: { onlyVisible?: boolean } = {}): Promise<ProductCategoryView[]> {
  const filter = options.onlyVisible
    ? { enabled: { $ne: false }, purchasable: { $ne: false } }
    : {}
  const rows = await ProductCategoryModel.find(filter).sort({ sortOrder: 1, createdAt: -1 }).lean().exec()
  return (rows as any[]).map(toCategoryView)
}

export async function createProductCategory(
  payload: { description?: unknown; enabled?: unknown; name?: unknown; purchasable?: unknown; sortOrder?: unknown },
  ...
  const created = await ProductCategoryModel.create({
    ...
    purchasable: payload.purchasable === undefined ? true : payload.purchasable !== false,
...
export async function updateProductCategory(
  ...
  const updated = await ProductCategoryModel.findByIdAndUpdate(
    categoryId,
    {
      ...
      purchasable: payload.purchasable === undefined ? current.purchasable !== false : payload.purchasable !== false,
    },
    { new: true },
  )
    .lean()
...
export async function ensureSkinProductCategory(): Promise<void> {
  const normalizedName = normalizeName(SKIN_PRODUCT_CATEGORY_NAME)
  await ProductCategoryModel.updateOne(
    { normalizedName },
    {
      $setOnInsert: {
        name: SKIN_PRODUCT_CATEGORY_NAME,
        description: '皮肤商品分类',
        sortOrder: 30,
        enabled: true,
        purchasable: true,
...
export async function getSkinProductCategory(): Promise<{ id: string; name: string } | null> {
  const normalizedName = normalizeName(SKIN_PRODUCT_CATEGORY_NAME)
  const row = await ProductCategoryModel.findOne({ normalizedName, enabled: true }).lean().exec()
  if (!row) {
    return null
