import type { GizmoAxisObject } from "../types";
import {
  Vector2,
  Raycaster,
  type Object3D,
...
export const intersectedObjects = (
  ...
  if (intersects.length > 0) {
    ...
    const closestDistance = first.distance;

    // Find all intersections within the threshold of the closest hit
    const nearHits = intersects.filter((hit) => hit.distance <= closestDistance + DISTANCE_THRESHOLD);
