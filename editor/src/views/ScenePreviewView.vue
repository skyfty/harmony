...
const instancedLodRuntimeEntriesCache: InstancedLodRuntimeEntry[] = []
let instancedLodRuntimeRevision = 0
let instancedLodRuntimeEntriesCacheRevision = -1
...
let instancedLodUpdateInFlight = false
let instancedLodUpdateQueued = false
...
function collectInstancedLodRuntimeEntries(): InstancedLodRuntimeEntry[] {
	if (instancedLodRuntimeEntriesCacheRevision === instancedLodRuntimeRevision) {
		return instancedLodRuntimeEntriesCache
	}
	...
	instancedLodRuntimeEntriesCache.length = 0
	instancedLodRuntimeEntriesCache.push(...entries)
	instancedLodRuntimeEntriesCacheRevision = instancedLodRuntimeRevision
	return instancedLodRuntimeEntriesCache
}
...
function areInstancedLodCameraMatricesEqualToSnapshot(
	camera: THREE.PerspectiveCamera,
	projectionSnapshot: Float32Array,
	matrixWorldInverseSnapshot: Float32Array,
): boolean {
	const projectionMatrix = camera.projectionMatrix.elements
	const matrixWorldInverse = camera.matrixWorldInverse.elements
	for (let i = 0; i < 16; i += 1) {
		if (projectionMatrix[i] !== projectionSnapshot[i]) {
			return false
		}
		if (matrixWorldInverse[i] !== matrixWorldInverseSnapshot[i]) {
			return false
		}
	}
	return true
...
async function updateInstancedCullingAndLod(camera: THREE.PerspectiveCamera | null): Promise<void> {
	if (instancedLodUpdateInFlight) {
		instancedLodUpdateQueued = true
		return
	}
	instancedLodUpdateInFlight = true
	try {
		if (!camera) {
			return
		}
	...
	const plannedRevision = instancedLodRuntimeRevision
	const cullingRequest = buildInstancedLodCullingRequestForFrame(camera, lodEntries)
	const cullingRequestProjectionMatrix = new Float32Array(cullingRequest.cameraProjectionMatrix)
	const cullingRequestMatrixWorldInverse = new Float32Array(cullingRequest.cameraMatrixWorldInverse)
	...
	if (
		!areInstancedLodCameraMatricesEqualToSnapshot(camera, cullingRequestProjectionMatrix, cullingRequestMatrixWorldInverse)
		|| instancedLodRuntimeRevision !== plannedRevision
	) {
		return
	}
	...
	lodEntries.forEach((entry, index) => {
		...
		if (!desiredTarget || !desiredTarget.assetId || !desiredTarget.key) {
			return
		}
		...
		instancedLodUpdateInFlight = false
		if (instancedLodUpdateQueued) {
			instancedLodUpdateQueued = false
			void updateInstancedCullingAndLod(camera)
		}
	}
}
...
function syncSceneNodeLocalTransformFromObject(node: SceneNode, object: THREE.Object3D): void {
	if (node.position) {
		node.position.x = object.position.x; node.position.y = object.position.y; node.position.z = object.position.z
	}
	const euler = new THREE.Euler().setFromQuaternion(object.quaternion, 'XYZ')
	if (node.rotation) { node.rotation.x = euler.x; node.rotation.y = euler.y; node.rotation.z = euler.z }
}
...
async function restoreControlNodeRuntime(transitionPreset: Extract<BehaviorRuntimeEvent, { type: 'control-node-restore' }>['transitionPreset'] = 'quantum'): Promise<boolean> {
	...
	if (vehicleDriveState.active) stopVehicleDriveMode({ resolution: { type: 'abort', message: 'Control node restored.' }, preserveCamera: true })
	applyMoveToObjectWorldPose(mainObject, worldPosition, worldQuaternion)
	...
	removeNodeSubtree(snapshot.temporaryNodeId)
	rebuildPreviewNodeMap(currentDocument)
	await syncPhysicsBodiesForDocument(currentDocument)
	refreshAnimations()
...
async function switchControlNodeRuntimePrefab(
	...
	if (runtimePrefabControlSwitchInFlight || !currentDocument || !scene || !rootGroup) return false
	...
	try {
		...
		if (!mainNode || !currentObject) return false
		const resolvedMainNodeId = mainNodeId
		const resolvedCurrentNodeId = currentNodeId
		const binding = resolveSteerBindingByTargetNodeId(currentDocument, resolvedCurrentNodeId)
		if (!binding) return false
		const raw = await loadTextAssetContent(assetId)
		if (!raw) return false
	...
	if (!instanced?.cloned.root.id) return false
		...
		if (!existing) {
			latestControlNodeRestoreSnapshot = {
				...
				mainVisible: nodeObjectMap.get(resolvedMainNodeId)?.visible !== false,
				componentEnabled: setControlNodeComponentEnabled(mainNode, false),
			}
		} else if (existing.temporaryNodeId) {
			removeSceneNodeById(currentDocument.nodes, existing.temporaryNodeId)
			removeNodeSubtree(existing.temporaryNodeId)
		}
		const mainObject = nodeObjectMap.get(resolvedMainNodeId) ?? null
		if (mainObject) mainObject.visible = false
		currentDocument.nodes = [...(currentDocument.nodes ?? []), effectiveNode]
		scene.add(instanced.sceneRootObject); registerSubtree(instanced.sceneRootObject); rebuildPreviewNodeMap(currentDocument)
		binding.steerComponent.props = clampSteerComponentProps({ ...binding.steerComponent.props, targetType: event.targetType, targetNodeId: resolvedEffectiveNodeId })
		syncSteerBindingIndexForNode(binding.steerNode); attachControlNodePrefabMetadata(effectiveNode, assetId)
		latestControlNodeRestoreSnapshot!.temporaryNodeId = effectiveNodeId
		await syncPhysicsBodiesForDocument(currentDocument)
		if (isCharacter) {
			pendingDefaultCharacterControlNodeId.value = effectiveNodeId; resetCharacterControlInputs(); resetProtagonistPoseState()
			refreshCharacterControllerAnimationRuntimeEntries(); refreshCharacterPathFollowRuntimeEntries()
		} else {
			const driveResult = startVehicleDriveMode(buildDefaultSteerDriveEvent(effectiveNodeId))
			if (!driveResult.success) return false
		}
		refreshAnimations()
		succeeded = true
...
function resolveBaseCameraOffsetForNode(nodeId: string): { distance: number; height: number } | null {
	...
	if (vehicleComponent) {
		...
		return {
			...
			height: props.cameraFollowHeight,
		}
	}
...
function handlePlayAnimationEvent(event: Extract<BehaviorRuntimeEvent, { type: 'play-animation' }>) {
	handleBehaviorPlayAnimationEvent(event, {
		...
		warnNoTarget: () => console.warn('[ScenePreview] Play animation skipped: no target node'),
		warnTargetUnavailable: (nodeId) => console.warn('[ScenePreview] Play animation skipped: target node does not expose animations', {
			nodeId,
		}),
		warnClipNotFound: (nodeId, clipName) => console.warn('[ScenePreview] Play animation skipped: clip not found', {
			targetNodeId: nodeId,
			requestedName: clipName,
		}),
		warnFailedToStart: () => console.warn('[ScenePreview] Play animation skipped: unable to start animation.'),
		warnFailedToStop: (error) => console.warn('[ScenePreview] Failed to stop animation action', error),
	})
}
...
function handleTriggerBehaviorEvent(event: Extract<BehaviorRuntimeEvent, { type: 'trigger-behavior' }>) {
	dispatchPerformBehaviorEvent(event, {
		warnMissingTarget: (message) => console.warn(`[ScenePreview] Trigger behavior skipped: ${message}`),
		triggerBehaviorAction,
		processBehaviorEvents,
	})
}
...
function handleCameraOffsetEvent(event: Extract<BehaviorRuntimeEvent, { type: 'camera-offset' }>) {
...
function handleCameraRestoreEvent(event: Extract<BehaviorRuntimeEvent, { type: 'camera-restore' }>) {
	const targetNodeId = resolveCameraOffsetTargetNodeId() ?? event.targetNodeId ?? event.nodeId ?? null
	if (!targetNodeId) {
...
function initRenderer() {
	...
	physicsCollisionDebugRuntime.setVisible(isPhysicsCollisionDebugVisible.value)
...
function applyPhysicsEnvironmentSettings(settings: EnvironmentSettings) {
	...
	physicsCollisionDebugRuntime.setEngine(
		physicsEnvironmentEnabled.value
			? resolveScenePreviewPhysicsCollisionDebugEngine(currentPhysicsBridgePreference.value)
			: null,
	)
...
async function applyBackgroundSettings(
	...
	if (background.mode === 'skycube') {
		...
		if (!isSkyCubeArchiveExtension(extension)) {
			const hdriKey = computeEnvironmentAssetReloadKey(normalizedAssetId)
			if (backgroundTexture && backgroundAssetId === normalizedAssetId && backgroundAssetKey === hdriKey && backgroundTextureSourceKind === 'texture') {
				scene.background = backgroundTexture
				return true
			}
			const loaded = await loadEnvironmentTextureFromAsset(normalizedAssetId)
			...
			backgroundAssetId = normalizedAssetId
		...
		if (!zipUrl) {
			disposeZipRef?.()
			console.warn('[ScenePreview] SkyCube zip URL unavailable', normalizedAssetId)
			return false
		}
		...
		try {
			...
			disposeZipRef?.()
			console.warn('[ScenePreview] Failed to fetch SkyCube zip', normalizedAssetId, error)
			return false
		} finally {
			disposeZipRef?.()
		}
		...
		try {
			...
			console.warn('[ScenePreview] Failed to unzip SkyCube zip', normalizedAssetId, error)
			return false
		}
...
function resolveScenePreviewPhysicsCollisionDebugEngine(
	preference: PhysicsBackendPreference | undefined,
): 'ammo' | 'cannon' {
...
function updateScenePreviewPhysicsBridgeIndex(document: SceneJsonExportDocument, asset: PhysicsSceneAsset): void {
...
watch(isPhysicsCollisionDebugVisible, (visible) => {
	physicsCollisionDebugRuntime.setVisible(visible)
})
