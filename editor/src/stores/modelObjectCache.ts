...
function expandHandle(handle: InstancedMeshHandle): number {
  const nextIndex = handle.capacity
  const nextCapacity = handle.capacity + DEFAULT_INSTANCE_CAPACITY
  const nextMesh = new InstancedMesh(
    handle.mesh.geometry.clone(),
    handle.mesh.material,
    nextCapacity,
  )
  nextMesh.instanceMatrix.setUsage(DynamicDrawUsage)
  for (let index = 0; index < handle.mesh.count; index += 1) {
    handle.mesh.getMatrixAt(index, tempMatrix)
    nextMesh.setMatrixAt(index, tempMatrix)
  }
  nextMesh.count = handle.mesh.count
  nextMesh.castShadow = handle.mesh.castShadow
  nextMesh.receiveShadow = handle.mesh.receiveShadow
  nextMesh.name = handle.mesh.name
  const parent = handle.mesh.parent
  if (parent) {
    const meshIndex = parent.children.indexOf(handle.mesh)
    if (meshIndex >= 0) {
      parent.children.splice(meshIndex, 1, nextMesh)
      nextMesh.parent = parent
    }
  }
  const handleId = handle.id
  handle.mesh.dispose()
  handle.mesh = nextMesh
  handle.capacity = nextCapacity
  handle.freeSlots = []
  handle.nodeByIndex.forEach((nodeId, index) => {
    nextMesh.setMatrixAt(index, tempMatrix.identity())
    nextMesh.instanceMatrix.needsUpdate = true
  })
  meshHandleLookup.set(handleId, handle)
  instancedMeshListeners.forEach((listener) => listener(nextMesh, nextMesh.userData.assetId))
  return nextIndex
}
