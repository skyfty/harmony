import { extractSkycubeZipFaces } from '@schema/skyCubeTexture'
...
console.info('[harmony-editor][skycube-zip][worker] module loaded')
...
;(self as unknown as Worker).addEventListener('message', (event: MessageEvent<ExtractRequest>) => {
  ...
  try {
    console.info('[harmony-editor][skycube-zip][worker] start', {
    ...
    console.info('[harmony-editor][skycube-zip][worker] complete', {
      requestId: message.requestId,
      faceCount: facesInOrder.length,
      missingFaces: extracted.missingFaces.length,
    })
  } catch (error) {
    console.warn('[harmony-editor][skycube-zip][worker] error', {
      requestId: message.requestId,
