    ...
    >
      <template #item="{ item, props: itemProps }">
        <v-list-item v-bind="itemProps">
          ...
          <template #subtitle>
            <span v-if="item.raw.description" class="series-item__description">
      ...
      <template #append-item>
        <v-divider v-if="allowCreate && searchLabel"></v-divider>
        <v-list-item
    ...
    <v-dialog v-model="createDialog" max-width="420">
      <v-card class="create-series-dialog__popover">
        ...
        <v-card-text>
          ...
          <v-textarea v-model="createForm.description" label="系列描述 (可选)" rows="3" />
...
const items = computed(() =>
  [...props.seriesOptions].sort((a, b) => (a.name ?? '').localeCompare(b.name ?? '', 'zh-CN')),
)
