...
const billboardComponent = computed(
  () => selectedNode.value?.components?.[BILLBOARD_COMPONENT_TYPE] as
    | SceneNodeComponentState<BillboardComponentProps>
    | undefined,
)

const componentEnabled = computed(() => billboardComponent.value?.enabled !== false)

const localProps = reactive({
  assetId: '',
  adaptation: 'fit' as BillboardComponentProps['adaptation'],
})

const adaptationOptions: Array<{ label: string; value: BillboardComponentProps['adaptation'] }> = [
  { label: 'Fit', value: 'fit' },
...
watch(
  () => billboardComponent.value?.props,
  (props) => {
    const next = props as Partial<BillboardComponentProps> | undefined
    localProps.assetId = normalizeAssetId(next?.assetId as string | undefined)
...
function handleToggleComponent() {
  const component = billboardComponent.value
...
async function handleClearAsset() {
  ...
  await sceneStore.applyBillboardAsset(nodeId, component.id, '', { updateMaterial: true })
}
...
async function handleDrop(event: DragEvent) {
  ...
  await sceneStore.applyBillboardAsset(nodeId, component.id, normalizedId, { updateMaterial: true })
}
...
<template>
  <v-expansion-panel value="billboard">
    <v-expansion-panel-title>
      <div class="display-board-panel__header">
        <span class="display-board-panel__title">Billboard</span>
        <v-spacer />
        <v-menu
          v-if="billboardComponent"
          location="bottom end"
          origin="auto"
