...
const LANDFORM_LINE_PREVIEW_RADIUS = 0.06
const LANDFORM_LINE_PREVIEW_HALO_RADIUS = 0.11
const LANDFORM_LINE_PREVIEW_POINT_RADIUS = 0.12
...
function updateLinePreviewGroup(group: THREE.Group, points: THREE.Vector3[]): void {
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    const halo = buildSegmentMesh(start, end, LANDFORM_LINE_PREVIEW_HALO_RADIUS, 0x16130d, 0.72, 140)
    const core = buildSegmentMesh(start, end, LANDFORM_LINE_PREVIEW_RADIUS, 0xffcc80, 1, 141)
    if (halo) {
      group.add(halo)
    }
    if (core) {
  ...
  points.forEach((point) => {
    addPointMarker(point, LANDFORM_LINE_PREVIEW_POINT_RADIUS, 0x16130d, 0.72, 142)
    addPointMarker(point, LANDFORM_LINE_PREVIEW_POINT_RADIUS * 0.58, 0xfff3e0, 1, 143)
  })
}
