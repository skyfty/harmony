...
const FLOOR_LINE_PREVIEW_RADIUS = 0.06
const FLOOR_LINE_PREVIEW_HALO_RADIUS = 0.11
const FLOOR_LINE_PREVIEW_POINT_RADIUS = 0.12
...
function createFloorLinePreviewGroup(points: THREE.Vector3[]): THREE.Group {
  ...
  updateFloorLinePreviewGroup(group, points)
  return group
}
...
function updateFloorLinePreviewGroup(group: THREE.Group, points: THREE.Vector3[]): void {
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    const halo = buildSegmentMesh(start, end, FLOOR_LINE_PREVIEW_HALO_RADIUS, 0x101418, 0.72, 140)
    const core = buildSegmentMesh(start, end, FLOOR_LINE_PREVIEW_RADIUS, 0xffd54f, 1, 141)
    if (halo) {
      group.add(halo)
    }
    if (core) {
  ...
  points.forEach((point) => {
    addPointMarker(point, FLOOR_LINE_PREVIEW_POINT_RADIUS, 0x101418, 0.72, 142)
    addPointMarker(point, FLOOR_LINE_PREVIEW_POINT_RADIUS * 0.58, 0xfff59d, 1, 143)
  })
}
