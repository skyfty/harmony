...
export type RegionPreviewSession = {
...
export type RegionBuildToolHandle = {
  getSession: () => RegionPreviewSession | null
...
function createRegionPreviewGroup(points: THREE.Vector3[]): THREE.Group {
  ...
  loop.renderOrder = 101
  group.userData.isRegionPreview = true
  group.userData.line = loop
  group.add(loop)
  return group
}

function updateRegionPreviewGroup(group: THREE.Group, points: THREE.Vector3[]): void {
  const line = group.userData.line as THREE.LineLoop | undefined
  if (!line) {
    return
  }
...
export function createRegionBuildTool(options: {
  ...
  sceneStore: {
    createRegionNode: (payload: { points: Vector3Like[] }) => { id: string } | null
    updateNodeUserData: (nodeId: string, userData: Record<string, unknown> | null | undefined) => unknown
  ...
  let session: RegionPreviewSession | null = null
  ...
  const resolvePlacementPoint = (event: PointerEvent, rawPoint: THREE.Vector3): THREE.Vector3 => {
    ...
    return snapped ? snapped.clone() : options.snapPoint(rawPoint.clone())
  }

  const hideStartIndicator = () => options.hideStartIndicator?.()

  const ensureSession = (): RegionPreviewSession => {
    if (session) {
      return session
    }
    session = {
  ...
  const showLockedStartIndicator = (point: THREE.Vector3 | null | undefined) => {
    ...
    options.showStartIndicator?.(point, { height: 0.2 })
  }
  ...
  const buildPreviewPoints = (target: RegionPreviewSession): THREE.Vector3[] => {
  ...
  const flushPreview = (scene: THREE.Scene | null): void => {
    previewDirty = false
    if (!scene || !session) {
      return
    }
    const previewPoints = buildPreviewPoints(session)
    ...
    const elevated = previewPoints.map((point) => new THREE.Vector3(point.x, point.y + PREVIEW_Y_OFFSET, point.z))
    if (!session.previewGroup) {
      session.previewGroup = createRegionPreviewGroup(elevated)
      options.rootGroup.add(session.previewGroup)
      return
    }
    updateRegionPreviewGroup(session.previewGroup, elevated)
  }
  ...
  const updateCursorPreview = (event: PointerEvent): void => {
    ...
    let next = resolvePlacementPoint(event, groundPointerHelper.clone())
    next.y = session.points[0]?.y ?? next.y
    if (options.isRelativeAngleSnapActive?.() && session.points.length >= 2) {
      const anchor = session.points[session.points.length - 1]
      const previous = session.points[session.points.length - 2]
      if (anchor && previous) {
        next = snapPointToRelativeAngleStep({
          anchor,
          previous,
          target: next,
  ...
  const finalize = (): void => {
    ...
    const created = options.sceneStore.createRegionNode({
      points: session.points.map((point) => ({ x: point.x, y: point.y, z: point.z })),
    })
    if (created) {
      options.sceneStore.updateNodeUserData(created.id, mergeUserDataWithDynamicMeshBuildShape(undefined, 'polygon'))
  ...
  const handlePlacementClick = (event: PointerEvent): boolean => {
    if (options.activeBuildTool.value !== 'region' || options.isAltOverrideActive()) {
      return false
    }
    if (!raycastPlacementPoint(event, groundPointerHelper)) {
      return false
    ...
    point.y = current.points[0]?.y ?? point.y
  ...
  return {
    ...
    handlePointerUp: (event: PointerEvent) => {
      ...
      if (event.button === 0) {
        ...
        if (handled) {
          if (isLeftDoubleClick(event) && session?.points.length >= 3) {
            finalize()
          }
