...
import type { PointerDownResult, RoadVertexDragState } from './SceneViewportPointerDownTypes'
      ...
      if (handleHit) {
        ...
        if (node?.dynamicMesh?.type === 'Road' && runtime) {
          const roadGroupCandidate =
            (runtime.userData?.roadGroup as THREE.Object3D | undefined) ??
            runtime.getObjectByName('RoadGroup') ??
            null
