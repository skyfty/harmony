...
export type BoundaryWallBuildToolSession = {
  targetNodeId: string
...
export type BoundaryWallBuildToolHandle = {
  getSession: () => BoundaryWallBuildToolSession | null
...
function createPreviewGroup(points: THREE.Vector3[]): THREE.Group {
  ...
  group.name = '__BoundaryWallPreview'
  ...
  group.userData.isBoundaryWallPreview = true
  group.add(line)
  return group
}
...
export function createBoundaryWallBuildTool(options: {
  ...
  commitLoop: (targetNodeId: string, points: THREE.Vector3[]) => boolean
  onCommitted?: () => void
}): BoundaryWallBuildToolHandle {
  const groundPointerHelper = new THREE.Vector3()
  let session: BoundaryWallBuildToolSession | null = null
  ...
  const alignPointYToSession = (point: THREE.Vector3, target: BoundaryWallBuildToolSession | null): THREE.Vector3 => {
  ...
  const ensureSession = (): BoundaryWallBuildToolSession | null => {
    if (session) {
      return session
    }
    ...
    session = { targetNodeId, points: [], previewEnd: null, previewGroup: null }
    return session
  }

  const getPreviewPoints = (target: BoundaryWallBuildToolSession | null): THREE.Vector3[] => {
    if (!target) {
      return []
    }
  ...
  const finalize = () => {
    ...
    const committed = options.commitLoop(session.targetNodeId, session.points.map((point) => point.clone()))
    clearSession()
    if (committed) {
      options.onCommitted?.()
    }
  ...
  return {
    ...
    handlePointerDown: (event) => {
      if (options.activeBuildTool.value !== 'boundaryWall') {
        return false
      }
