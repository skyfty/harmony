...
function noise3D(x: number, y: number, z: number) {
  ...
  return lerp(
    ...
    lerp(
      v,
      lerp(u, grad(P[AA] ?? 0, x, y, z), grad(P[BA] ?? 0, x - 1, y, z)),
      lerp(u, grad(P[AB] ?? 0, x, y - 1, z), grad(P[BB] ?? 0, x - 1, y - 1, z)),
    ),
    lerp(
      v,
      lerp(u, grad(P[AA + 1] ?? 0, x, y, z - 1), grad(P[BA + 1] ?? 0, x - 1, y, z - 1)),
      lerp(u, grad(P[AB + 1] ?? 0, x, y - 1, z - 1), grad(P[BB + 1] ?? 0, x - 1, y - 1, z - 1)),
    ),
  )
}
...
export class SceneCloudRenderer {
  ...
  setSunPosition(position: THREE.Vector3): void {
    ...
    if (this.volumetricMaterial) {
      ...
      const sunUniform = uniforms?.['uSunPos']
      if (sunUniform?.value?.copy) {
        sunUniform.value.copy(this.sunPosition)
      }
    }
  }
