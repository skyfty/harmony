import {
  clampParticleSystemComponentProps,
  cloneParticleSystemComponentProps,
  DEFAULT_PARTICLE_BUDGET,
  DEFAULT_PARTICLE_EXPOSED_PARAMS,
  type ParticleSystemComponentProps,
} from './particleSchema'

export type ParticlePresetDefinition = {
  id: string
  label: string
  description: string
  category: ParticlePresetCategory
  subcategory: string
  props: ParticleSystemComponentProps
}

export type ParticlePresetCategory =
  | 'Weather'
  | 'Environment'
  | 'Combat'
  | 'Magic'
  | 'Support'
  | 'Interaction'

export type ParticlePresetGroup = {
  category: ParticlePresetCategory
  label: string
  subgroups: ParticlePresetSubgroup[]
}

export type ParticlePresetSubgroup = {
  subcategory: string
  label: string
  presets: ParticlePresetDefinition[]
}

type ParticlePresetEntry = Omit<ParticlePresetDefinition, 'category'>

const PARTICLE_PRESET_CATEGORY_ORDER: ParticlePresetCategory[] = [
  'Weather',
  'Environment',
  'Combat',
  'Magic',
  'Support',
  'Interaction',
]

function resolveParticlePresetCategory(id: string): ParticlePresetCategory {
  if (id.includes('rain') || id.includes('snow') || id.includes('leaf') || id.includes('wind')) {
    return 'Weather'
  }
  if (id.includes('ground') || id.includes('smoke') || id.includes('dust') || id.includes('bubble') || id.includes('embers') || id.includes('firefly')) {
    return 'Environment'
  }
  if (id.includes('spark') || id.includes('impact') || id.includes('explosion') || id.includes('thunder') || id.includes('beam') || id.includes('energyTrail') || id.includes('waterSplash') || id.includes('lavaSplash')) {
    return 'Combat'
  }
  if (id.includes('magic') || id.includes('portal') || id.includes('teleport') || id.includes('frost') || id.includes('poison')) {
    return 'Magic'
  }
  if (id.includes('heal') || id.includes('shield') || id.includes('levelUp') || id.includes('buff') || id.includes('debuff')) {
    return 'Support'
  }
  return 'Interaction'
}

function resolveParticlePresetSubcategory(id: string): string {
  if (id.includes('rain')) {
    return 'Rain'
  }
  if (id.includes('snow')) {
    return 'Snow'
  }
  if (id.includes('leaf')) {
    return 'Leaves'
  }
  if (id.includes('wind')) {
    return 'Wind'
  }
  if (id.includes('ground')) {
    return 'Ground'
  }
  if (id.includes('smoke')) {
    return 'Smoke'
  }
  if (id.includes('dust')) {
    return 'Dust'
  }
  if (id.includes('bubble')) {
    return 'Bubbles'
  }
  if (id.includes('embers')) {
    return 'Embers'
  }
  if (id.includes('firefly')) {
    return 'Fireflies'
  }
  if (id.includes('spark')) {
    return 'Sparks'
  }
  if (id.includes('impact') || id.includes('debris') || id.includes('explosion') || id.includes('thunder') || id.includes('beam')) {
    return 'Impact'
  }
  if (id.includes('energyTrail')) {
    return 'Trails'
  }
  if (id.includes('waterSplash') || id.includes('lavaSplash')) {
    return 'Elemental'
  }
  if (id.includes('magic')) {
    return 'Sparkles'
  }
  if (id.includes('portal') || id.includes('teleport')) {
    return 'Portal'
  }
  if (id.includes('frost') || id.includes('poison')) {
    return 'Status'
  }
  if (id.includes('heal') || id.includes('shield') || id.includes('levelUp') || id.includes('buff') || id.includes('debuff')) {
    return 'Support'
  }
  if (id.includes('coin') || id.includes('item')) {
    return 'Pickup'
  }
  return 'General'
}

const PRESETS: ParticlePresetEntry[] = [
  {
    id: 'groundAuraLite',
    label: 'Ground Aura Lite',
    description: 'Low-cost ring aura for ground highlights.',
    subcategory: 'Ground',
    props: clampParticleSystemComponentProps({
      presetId: 'groundAuraLite',
      budget: { ...DEFAULT_PARTICLE_BUDGET, maxParticles: 72, cullDistance: 20, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#46d9ff', emissionRate: 12, burstCount: 16, radius: 0.9, speed: 1.2, lifetime: 1.4 },
      emitters: [
        {
          id: 'ground_ring',
          shape: 'sphere',
          radius: 0.85,
          emissionRate: 12,
          emissionBursts: 16,
          maxParticles: 72,
          particleSize: 0.16,
          lifetime: 1.4,
          speed: 1.2,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 0.5,
          color: '#46d9ff',
          color2: '#a8f6ff',
          alphaStart: 0.8,
          alphaEnd: 0,
          scaleStart: 0.65,
          scaleEnd: 0.15,
          position: { x: 0, y: 0.05, z: 0 },
          size: { x: 0.1, y: 0.1, z: 0.1 },
        },
      ],
    }),
  },
  {
    id: 'sparkBurstLite',
    label: 'Spark Burst Lite',
    description: 'One-shot spark burst with capped mobile burst counts.',
    subcategory: 'Sparks',
    props: clampParticleSystemComponentProps({
      presetId: 'sparkBurstLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.15 },
      budget: { qualityTier: 'mini-safe', maxParticles: 48, spawnRateScale: 1, cullDistance: 18, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ffd16a', emissionRate: 0, burstCount: 28, radius: 0.15, speed: 4.5, lifetime: 0.7, particleSize: 0.09 },
      emitters: [
        {
          id: 'spark_burst',
          shape: 'point',
          position: { x: 0, y: 0.2, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.15,
          emissionRate: 0,
          emissionBursts: 28,
          maxParticles: 48,
          particleSize: 0.09,
          lifetime: 0.7,
          speed: 4.5,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.4,
          color: '#ffd16a',
          color2: '#ff8f4f',
          alphaStart: 0.95,
          alphaEnd: 0,
          scaleStart: 1,
          scaleEnd: 0.2,
        },
      ],
    }),
  },
  {
    id: 'smokeLoopLite',
    label: 'Smoke Loop Lite',
    description: 'Slow drifting smoke tuned for low-frequency updates.',
    subcategory: 'Smoke',
    props: clampParticleSystemComponentProps({
      presetId: 'smokeLoopLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 1, cullDistance: 22, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#c7d0d8', emissionRate: 8, burstCount: 10, radius: 0.45, speed: 0.8, lifetime: 2.2, particleSize: 0.26, opacity: 0.45 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'smoke_loop',
          shape: 'box',
          position: { x: 0, y: 0.2, z: 0 },
          size: { x: 0.35, y: 0.2, z: 0.35 },
          radius: 0.45,
          emissionRate: 8,
          emissionBursts: 10,
          maxParticles: 64,
          particleSize: 0.26,
          lifetime: 2.2,
          speed: 0.8,
          velocityMode: 'vector',
          direction: { x: 0.1, y: 1, z: 0.1 },
          spread: 0.45,
          color: '#c7d0d8',
          color2: '#eef2f5',
          alphaStart: 0.45,
          alphaEnd: 0,
          scaleStart: 0.8,
          scaleEnd: 1.45,
        },
      ],
    }),
  },
  {
    id: 'rainCurtainLite',
    label: 'Rain Curtain Lite',
    description: 'Soft falling rain streaks for weather ambience.',
    subcategory: 'Rain',
    props: clampParticleSystemComponentProps({
      presetId: 'rainCurtainLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 0.58, cullDistance: 44, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#9cd6f5', emissionRate: 12, burstCount: 8, radius: 1.55, speed: 4.4, lifetime: 1.6, particleSize: 0.05, opacity: 0.42 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'rain_field',
          shape: 'box',
          position: { x: 0, y: 4.2, z: 0 },
          size: { x: 5.6, y: 0.28, z: 5.6 },
          radius: 1.55,
          emissionRate: 12,
          emissionBursts: 8,
          maxParticles: 64,
          particleSize: 0.05,
          lifetime: 1.6,
          speed: 4.4,
          velocityMode: 'vector',
          direction: { x: 0, y: -1, z: 0 },
          spread: 0.04,
          physics: { force: { x: 0, y: -0.1, z: 0 } },
          color: '#9cd6f5',
          color2: '#e5f4fb',
          alphaStart: 0.78,
          alphaEnd: 0,
          scaleStart: 0.12,
          scaleEnd: 0.65,
        },
      ],
    }),
  },
  {
    id: 'rainSmallLite',
    label: 'Light Rain Lite',
    description: 'A lighter, smaller rain curtain for local weather pockets.',
    subcategory: 'Rain',
    props: clampParticleSystemComponentProps({
      presetId: 'rainSmallLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 40, spawnRateScale: 0.45, cullDistance: 38, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#a6def8', emissionRate: 7, burstCount: 4, radius: 1.05, speed: 4.1, lifetime: 1.4, particleSize: 0.042, opacity: 0.34 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'rain_field',
          shape: 'box',
          position: { x: 0, y: 3.8, z: 0 },
          size: { x: 4.2, y: 0.22, z: 4.2 },
          radius: 1.05,
          emissionRate: 7,
          emissionBursts: 4,
          maxParticles: 40,
          particleSize: 0.042,
          lifetime: 1.4,
          speed: 4.1,
          velocityMode: 'vector',
          direction: { x: 0, y: -1, z: 0 },
          spread: 0.035,
          physics: { force: { x: 0, y: -0.08, z: 0 } },
          color: '#a6def8',
          color2: '#e7f6fc',
          alphaStart: 0.74,
          alphaEnd: 0,
          scaleStart: 0.1,
          scaleEnd: 0.58,
        },
      ],
    }),
  },
  {
    id: 'rainHeavyLite',
    label: 'Heavy Rain Lite',
    description: 'Denser, stronger rain curtain for stormier ambience.',
    subcategory: 'Rain',
    props: clampParticleSystemComponentProps({
      presetId: 'rainHeavyLite',
      budget: { qualityTier: 'balanced', maxParticles: 96, spawnRateScale: 0.62, cullDistance: 52, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#86cfee', emissionRate: 16, burstCount: 10, radius: 1.85, speed: 5.2, lifetime: 1.7, particleSize: 0.054, opacity: 0.46 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'rain_field',
          shape: 'box',
          position: { x: 0, y: 5, z: 0 },
          size: { x: 8, y: 0.3, z: 8 },
          radius: 1.85,
          emissionRate: 16,
          emissionBursts: 10,
          maxParticles: 96,
          particleSize: 0.054,
          lifetime: 1.7,
          speed: 5.2,
          velocityMode: 'vector',
          direction: { x: 0, y: -1, z: 0 },
          spread: 0.05,
          physics: { force: { x: 0, y: -0.14, z: 0 } },
          color: '#86cfee',
          color2: '#d8ebf6',
          alphaStart: 0.84,
          alphaEnd: 0,
          scaleStart: 0.1,
          scaleEnd: 0.68,
        },
      ],
    }),
  },
  {
    id: 'snowDriftLite',
    label: 'Snow Drift Lite',
    description: 'Gentle snow flakes drifting downward with soft visibility.',
    subcategory: 'Snow',
    props: clampParticleSystemComponentProps({
      presetId: 'snowDriftLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 0.45, cullDistance: 48, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#f6fbff', emissionRate: 5, burstCount: 4, radius: 1.5, speed: 0.14, lifetime: 10.4, particleSize: 0.11, opacity: 0.82 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'snow_field',
          shape: 'box',
          position: { x: 0, y: 5.4, z: 0 },
          size: { x: 7.4, y: 0.32, z: 7.4 },
          radius: 1.5,
          emissionRate: 5,
          emissionBursts: 4,
          maxParticles: 64,
          particleSize: 0.11,
          lifetime: 10.4,
          speed: 0.14,
          velocityMode: 'radial',
          direction: { x: 0, y: -1, z: 0 },
          spread: 0.95,
          physics: { force: { x: 0, y: -0.03, z: 0 } },
          color: '#f6fbff',
          color2: '#dde8f3',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.14,
          scaleEnd: 1.28,
        },
      ],
    }),
  },
  {
    id: 'snowLightLite',
    label: 'Light Snow Lite',
    description: 'A sparse, softer snowfall for lighter winter ambience.',
    subcategory: 'Snow',
    props: clampParticleSystemComponentProps({
      presetId: 'snowLightLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 40, spawnRateScale: 0.38, cullDistance: 42, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#f8fdff', emissionRate: 2, burstCount: 2, radius: 1.35, speed: 0.1, lifetime: 11.2, particleSize: 0.1, opacity: 0.72 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'snow_field',
          shape: 'box',
          position: { x: 0, y: 5.6, z: 0 },
          size: { x: 6.4, y: 0.34, z: 6.4 },
          radius: 1.35,
          emissionRate: 2,
          emissionBursts: 2,
          maxParticles: 200,
          particleSize: 0.1,
          lifetime: 11.2,
          speed: 0.1,
          velocityMode: 'radial',
          direction: { x: 0, y: -1, z: 0 },
          spread: 1.1,
          physics: { force: { x: 0, y: -0.02, z: 0 } },
          color: '#f8fdff',
          color2: '#e1ebf5',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.12,
          scaleEnd: 1.18,
        },
      ],
    }),
  },
  {
    id: 'snowBlizzardLite',
    label: 'Blizzard Lite',
    description: 'A denser snowfall with more drift for stormy winter weather.',
    subcategory: 'Snow',
    props: clampParticleSystemComponentProps({
      presetId: 'snowBlizzardLite',
      budget: { qualityTier: 'balanced', maxParticles: 900, spawnRateScale: 1, cullDistance: 56, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#f9feff', emissionRate: 120, burstCount: 36, radius: 2.1, speed: 0.2, lifetime: 9.5, particleSize: 0.04, opacity: 0.88 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'snow_field_nw',
          shape: 'box',
          position: { x: -4.8, y: 6.4, z: -4.8 },
          size: { x: 8.8, y: 0.34, z: 8.8 },
          radius: 2.1,
          emissionRate: 120,
          emissionBursts: 36,
          maxParticles: 900,
          particleSize: 0.04,
          lifetime: 9.5,
          speed: 0.2,
          velocityMode: 'radial',
          direction: { x: 0, y: -1, z: 0 },
          spread: 1.25,
          physics: { force: { x: 0, y: -0.04, z: 0 } },
          color: '#f9feff',
          color2: '#deebf4',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.04,
          scaleEnd: 0.55,
        },
        {
          id: 'snow_field_ne',
          shape: 'box',
          position: { x: 4.8, y: 6.4, z: -4.8 },
          size: { x: 8.8, y: 0.34, z: 8.8 },
          radius: 2.1,
          emissionRate: 120,
          emissionBursts: 36,
          maxParticles: 900,
          particleSize: 0.04,
          lifetime: 9.5,
          speed: 0.2,
          velocityMode: 'radial',
          direction: { x: 0, y: -1, z: 0 },
          spread: 1.25,
          physics: { force: { x: 0, y: -0.04, z: 0 } },
          color: '#f9feff',
          color2: '#deebf4',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.04,
          scaleEnd: 0.55,
        },
        {
          id: 'snow_field_sw',
          shape: 'box',
          position: { x: -4.8, y: 6.4, z: 4.8 },
          size: { x: 8.8, y: 0.34, z: 8.8 },
          radius: 2.1,
          emissionRate: 120,
          emissionBursts: 36,
          maxParticles: 900,
          particleSize: 0.04,
          lifetime: 9.5,
          speed: 0.2,
          velocityMode: 'radial',
          direction: { x: 0, y: -1, z: 0 },
          spread: 1.25,
          physics: { force: { x: 0, y: -0.04, z: 0 } },
          color: '#f9feff',
          color2: '#deebf4',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.04,
          scaleEnd: 0.55,
        },
        {
          id: 'snow_field_se',
          shape: 'box',
          position: { x: 4.8, y: 6.4, z: 4.8 },
          size: { x: 8.8, y: 0.34, z: 8.8 },
          radius: 2.1,
          emissionRate: 120,
          emissionBursts: 36,
          maxParticles: 900,
          particleSize: 0.04,
          lifetime: 9.5,
          speed: 0.2,
          velocityMode: 'radial',
          direction: { x: 0, y: -1, z: 0 },
          spread: 1.25,
          physics: { force: { x: 0, y: -0.04, z: 0 } },
          color: '#f9feff',
          color2: '#deebf4',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.04,
          scaleEnd: 0.55,
        },
      ],
    }),
  },
  {
    id: 'gravityRainLite',
    label: 'Gravity Rain Lite',
    description: 'A clearer rain streak preset with more obvious downward motion and softer spread.',
    subcategory: 'Rain',
    props: clampParticleSystemComponentProps({
      presetId: 'gravityRainLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.6, cullDistance: 52, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#9bd5f5', emissionRate: 10, burstCount: 8, radius: 2.1, speed: 4.6, lifetime: 1.7, particleSize: 0.05, opacity: 0.46 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'rain_field',
          shape: 'box',
          position: { x: 0, y: 5.2, z: 0 },
          size: { x: 7.6, y: 0.34, z: 7.6 },
          radius: 2.1,
          emissionRate: 10,
          emissionBursts: 8,
          maxParticles: 72,
          particleSize: 0.05,
          lifetime: 1.7,
          speed: 4.6,
          velocityMode: 'vector',
          direction: { x: 0, y: -1, z: 0 },
          spread: 0.035,
          physics: { force: { x: 0, y: -0.12, z: 0 } },
          color: '#9bd5f5',
          color2: '#e7f4fb',
          alphaStart: 0.78,
          alphaEnd: 0,
          scaleStart: 0.12,
          scaleEnd: 0.62,
        },
      ],
    }),
  },
  {
    id: 'gravitySnowLite',
    label: 'Gravity Snow Lite',
    description: 'A soft snowfall preset with slower drift and clearly visible flakes.',
    subcategory: 'Snow',
    props: clampParticleSystemComponentProps({
      presetId: 'gravitySnowLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.5, cullDistance: 60, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#f9feff', emissionRate: 6, burstCount: 4, radius: 2.4, speed: 0.12, lifetime: 11.4, particleSize: 0.13, opacity: 0.82 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'snow_field',
          shape: 'box',
          position: { x: 0, y: 6.2, z: 0 },
          size: { x: 10, y: 0.4, z: 10 },
          radius: 2.4,
          emissionRate: 6,
          emissionBursts: 4,
          maxParticles: 72,
          particleSize: 0.13,
          lifetime: 11.4,
          speed: 0.12,
          velocityMode: 'radial',
          direction: { x: 0, y: -1, z: 0 },
          spread: 1.35,
          physics: { force: { x: 0, y: -0.015, z: 0 } },
          color: '#f9feff',
          color2: '#e1edf5',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.12,
          scaleEnd: 1.28,
        },
      ],
    }),
  },
  {
    id: 'leafFallLite',
    label: 'Leaf Fall Lite',
    description: 'Autumn leaves gently falling through the air.',
    subcategory: 'Leaves',
    props: clampParticleSystemComponentProps({
      presetId: 'leafFallLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 0.6, cullDistance: 36, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#d89a43', emissionRate: 10, burstCount: 12, radius: 1.1, speed: 1.1, lifetime: 5.5, particleSize: 0.22, opacity: 0.9 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'leaf_canopy',
          shape: 'box',
          position: { x: 0, y: 4.2, z: 0 },
          size: { x: 4.8, y: 0.3, z: 4.8 },
          radius: 1.1,
          emissionRate: 10,
          emissionBursts: 12,
          maxParticles: 64,
          particleSize: 0.22,
          lifetime: 5.5,
          speed: 1.1,
          velocityMode: 'vector',
          direction: { x: 0.12, y: -1, z: 0.08 },
          spread: 1.0,
          color: '#d89a43',
          color2: '#8c5c2b',
          alphaStart: 0.92,
          alphaEnd: 0,
          scaleStart: 1,
          scaleEnd: 0.7,
        },
      ],
    }),
  },
  {
    id: 'embersRiseLite',
    label: 'Embers Rise Lite',
    description: 'Warm ember sparks drifting upward from heat sources.',
    subcategory: 'Embers',
    props: clampParticleSystemComponentProps({
      presetId: 'embersRiseLite',
      budget: { qualityTier: 'balanced', maxParticles: 80, spawnRateScale: 0.8, cullDistance: 26, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ffab4c', emissionRate: 16, burstCount: 18, radius: 0.55, speed: 1.8, lifetime: 2.1, particleSize: 0.1, opacity: 0.78 },
      emitters: [
        {
          id: 'ember_core',
          shape: 'sphere',
          position: { x: 0, y: 0.15, z: 0 },
          size: { x: 0.2, y: 0.2, z: 0.2 },
          radius: 0.5,
          emissionRate: 16,
          emissionBursts: 18,
          maxParticles: 80,
          particleSize: 0.1,
          lifetime: 2.1,
          speed: 1.8,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 0.95,
          color: '#ffab4c',
          color2: '#ffe08a',
          alphaStart: 0.8,
          alphaEnd: 0,
          scaleStart: 0.55,
          scaleEnd: 0.2,
        },
      ],
    }),
  },
  {
    id: 'dustMotesLite',
    label: 'Dust Motes Lite',
    description: 'Tiny floating dust motes for interiors and caves.',
    subcategory: 'Dust',
    props: clampParticleSystemComponentProps({
      presetId: 'dustMotesLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 56, spawnRateScale: 0.5, cullDistance: 20, updateHz: 20 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#d8d2c7', emissionRate: 6, burstCount: 8, radius: 0.8, speed: 0.35, lifetime: 6, particleSize: 0.06, opacity: 0.28 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'dust_volume',
          shape: 'box',
          position: { x: 0, y: 1, z: 0 },
          size: { x: 3, y: 1.5, z: 3 },
          radius: 0.8,
          emissionRate: 6,
          emissionBursts: 8,
          maxParticles: 56,
          particleSize: 0.06,
          lifetime: 6,
          speed: 0.35,
          velocityMode: 'vector',
          direction: { x: 0.03, y: 0.22, z: 0.02 },
          spread: 0.3,
          color: '#d8d2c7',
          color2: '#f1ece3',
          alphaStart: 0.28,
          alphaEnd: 0,
          scaleStart: 0.7,
          scaleEnd: 1.3,
        },
      ],
    }),
  },
  {
    id: 'bubbleRiseLite',
    label: 'Bubble Rise Lite',
    description: 'Soft upward bubbles for underwater scenes.',
    subcategory: 'Bubbles',
    props: clampParticleSystemComponentProps({
      presetId: 'bubbleRiseLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 0.7, cullDistance: 24, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#b8f0ff', emissionRate: 10, burstCount: 12, radius: 0.45, speed: 0.9, lifetime: 4.4, particleSize: 0.16, opacity: 0.45 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'bubble_stream',
          shape: 'box',
          position: { x: 0, y: 0.2, z: 0 },
          size: { x: 0.35, y: 0.2, z: 0.35 },
          radius: 0.45,
          emissionRate: 10,
          emissionBursts: 12,
          maxParticles: 64,
          particleSize: 0.16,
          lifetime: 4.4,
          speed: 0.9,
          velocityMode: 'vector',
          direction: { x: 0.04, y: 1, z: 0.04 },
          spread: 0.5,
          color: '#b8f0ff',
          color2: '#f4feff',
          alphaStart: 0.35,
          alphaEnd: 0,
          scaleStart: 0.5,
          scaleEnd: 1.1,
        },
      ],
    }),
  },
  {
    id: 'fireflyGlowLite',
    label: 'Firefly Glow Lite',
    description: 'Tiny glowing dots for nighttime ambience.',
    subcategory: 'Fireflies',
    props: clampParticleSystemComponentProps({
      presetId: 'fireflyGlowLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 48, spawnRateScale: 0.55, cullDistance: 20, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ffe37a', emissionRate: 8, burstCount: 8, radius: 0.7, speed: 0.5, lifetime: 3.5, particleSize: 0.08, opacity: 0.75 },
      emitters: [
        {
          id: 'firefly_cloud',
          shape: 'sphere',
          position: { x: 0, y: 1.4, z: 0 },
          size: { x: 1, y: 1, z: 1 },
          radius: 0.65,
          emissionRate: 8,
          emissionBursts: 8,
          maxParticles: 48,
          particleSize: 0.08,
          lifetime: 3.5,
          speed: 0.5,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 1.4,
          color: '#ffe37a',
          color2: '#fff9cf',
          alphaStart: 0.75,
          alphaEnd: 0,
          scaleStart: 0.4,
          scaleEnd: 1.0,
        },
      ],
    }),
  },
  {
    id: 'magicSparkleLite',
    label: 'Magic Sparkle Lite',
    description: 'Small glitter sparkles for spells and enchanted UI.',
    subcategory: 'Sparkles',
    props: clampParticleSystemComponentProps({
      presetId: 'magicSparkleLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 0.75, cullDistance: 22, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#d5a8ff', emissionRate: 12, burstCount: 16, radius: 0.4, speed: 1.2, lifetime: 1.6, particleSize: 0.08, opacity: 0.85 },
      render: { textureAssetId: null, blendMode: 'additive', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'sparkle_core',
          shape: 'sphere',
          position: { x: 0, y: 0.3, z: 0 },
          size: { x: 0.15, y: 0.15, z: 0.15 },
          radius: 0.32,
          emissionRate: 12,
          emissionBursts: 16,
          maxParticles: 64,
          particleSize: 0.08,
          lifetime: 1.6,
          speed: 1.2,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 1.4,
          color: '#d5a8ff',
          color2: '#fff4ff',
          alphaStart: 0.9,
          alphaEnd: 0,
          scaleStart: 0.35,
          scaleEnd: 0.1,
        },
      ],
    }),
  },
  {
    id: 'healPulseLite',
    label: 'Heal Pulse Lite',
    description: 'Soft green healing pulse for support skills and recovery.',
    subcategory: 'Support',
    props: clampParticleSystemComponentProps({
      presetId: 'healPulseLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.8, cullDistance: 24, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#8df59b', emissionRate: 14, burstCount: 18, radius: 0.85, speed: 1.0, lifetime: 1.8, particleSize: 0.13, opacity: 0.8 },
      emitters: [
        {
          id: 'heal_ring',
          shape: 'sphere',
          position: { x: 0, y: 0.1, z: 0 },
          size: { x: 0.2, y: 0.2, z: 0.2 },
          radius: 0.8,
          emissionRate: 14,
          emissionBursts: 18,
          maxParticles: 200,
          particleSize: 0.13,
          lifetime: 1.8,
          speed: 1,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 0.7,
          color: '#8df59b',
          color2: '#e8ffef',
          alphaStart: 0.85,
          alphaEnd: 0,
          scaleStart: 0.5,
          scaleEnd: 0.18,
        },
      ],
    }),
  },
  {
    id: 'shieldAuraLite',
    label: 'Shield Aura Lite',
    description: 'Blue protective aura for buffs and defensive states.',
    subcategory: 'Support',
    props: clampParticleSystemComponentProps({
      presetId: 'shieldAuraLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.7, cullDistance: 24, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#6fc8ff', emissionRate: 10, burstCount: 12, radius: 1.0, speed: 0.7, lifetime: 2.4, particleSize: 0.12, opacity: 0.65 },
      render: { textureAssetId: null, blendMode: 'additive', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'shield_shell',
          shape: 'sphere',
          position: { x: 0, y: 0.6, z: 0 },
          size: { x: 0.25, y: 0.25, z: 0.25 },
          radius: 0.9,
          emissionRate: 10,
          emissionBursts: 12,
          maxParticles: 200,
          particleSize: 0.12,
          lifetime: 2.4,
          speed: 0.7,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 0.9,
          color: '#6fc8ff',
          color2: '#d8f5ff',
          alphaStart: 0.7,
          alphaEnd: 0,
          scaleStart: 0.55,
          scaleEnd: 0.18,
        },
      ],
    }),
  },
  {
    id: 'levelUpBurstLite',
    label: 'Level Up Burst Lite',
    description: 'Celebratory vertical burst for level-up and rank-up moments.',
    subcategory: 'Support',
    props: clampParticleSystemComponentProps({
      presetId: 'levelUpBurstLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.15 },
      budget: { qualityTier: 'mini-safe', maxParticles: 48, spawnRateScale: 1, cullDistance: 20, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ffe27a', emissionRate: 0, burstCount: 24, radius: 0.2, speed: 3.2, lifetime: 1.0, particleSize: 0.11, opacity: 0.95 },
      emitters: [
        {
          id: 'level_up',
          shape: 'point',
          position: { x: 0, y: 0.2, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.2,
          emissionRate: 0,
          emissionBursts: 24,
          maxParticles: 48,
          particleSize: 0.11,
          lifetime: 1.0,
          speed: 3.2,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 1.8,
          color: '#ffe27a',
          color2: '#fff3be',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.8,
          scaleEnd: 0.18,
        },
      ],
    }),
  },
  {
    id: 'coinPickupLite',
    label: 'Coin Pickup Lite',
    description: 'Gold burst for coins, pickups, and rewards.',
    subcategory: 'Pickup',
    props: clampParticleSystemComponentProps({
      presetId: 'coinPickupLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.1 },
      budget: { qualityTier: 'mini-safe', maxParticles: 40, spawnRateScale: 1, cullDistance: 18, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ffc84d', emissionRate: 0, burstCount: 16, radius: 0.18, speed: 2.6, lifetime: 0.8, particleSize: 0.08, opacity: 0.95 },
      emitters: [
        {
          id: 'coin_glint',
          shape: 'point',
          position: { x: 0, y: 0.15, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.18,
          emissionRate: 0,
          emissionBursts: 16,
          maxParticles: 40,
          particleSize: 0.08,
          lifetime: 0.8,
          speed: 2.6,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.2,
          color: '#ffc84d',
          color2: '#fff0aa',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.75,
          scaleEnd: 0.15,
        },
      ],
    }),
  },
  {
    id: 'itemPickupLite',
    label: 'Item Pickup Lite',
    description: 'Small white-gold pickup burst for inventory and loot.',
    subcategory: 'Pickup',
    props: clampParticleSystemComponentProps({
      presetId: 'itemPickupLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.1 },
      budget: { qualityTier: 'mini-safe', maxParticles: 40, spawnRateScale: 1, cullDistance: 18, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#fff7d1', emissionRate: 0, burstCount: 14, radius: 0.16, speed: 2.2, lifetime: 0.9, particleSize: 0.08, opacity: 0.9 },
      emitters: [
        {
          id: 'loot_glint',
          shape: 'point',
          position: { x: 0, y: 0.12, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.16,
          emissionRate: 0,
          emissionBursts: 14,
          maxParticles: 40,
          particleSize: 0.08,
          lifetime: 0.9,
          speed: 2.2,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.0,
          color: '#fff7d1',
          color2: '#ffffff',
          alphaStart: 0.95,
          alphaEnd: 0,
          scaleStart: 0.7,
          scaleEnd: 0.15,
        },
      ],
    }),
  },
  {
    id: 'impactDebrisLite',
    label: 'Impact Debris Lite',
    description: 'Small brown debris burst for hits and collision impacts.',
    subcategory: 'Impact',
    props: clampParticleSystemComponentProps({
      presetId: 'impactDebrisLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.12 },
      budget: { qualityTier: 'mini-safe', maxParticles: 48, spawnRateScale: 1, cullDistance: 18, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#c79a68', emissionRate: 0, burstCount: 20, radius: 0.2, speed: 3.1, lifetime: 0.9, particleSize: 0.07, opacity: 0.92 },
      emitters: [
        {
          id: 'debris_burst',
          shape: 'point',
          position: { x: 0, y: 0.1, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.2,
          emissionRate: 0,
          emissionBursts: 20,
          maxParticles: 48,
          particleSize: 0.07,
          lifetime: 0.9,
          speed: 3.1,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.5,
          physics: { force: { x: 0, y: -0.45, z: 0 } },
          color: '#c79a68',
          color2: '#8c6a49',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.6,
          scaleEnd: 0.2,
        },
      ],
    }),
  },
  {
    id: 'explosionFlashLite',
    label: 'Explosion Flash Lite',
    description: 'Bright orange explosion burst for combat and destruction.',
    subcategory: 'Impact',
    props: clampParticleSystemComponentProps({
      presetId: 'explosionFlashLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.08 },
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 1, cullDistance: 20, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ff9b53', emissionRate: 0, burstCount: 30, radius: 0.24, speed: 4.2, lifetime: 0.8, particleSize: 0.1, opacity: 0.98 },
      emitters: [
        {
          id: 'explosion_core',
          shape: 'sphere',
          position: { x: 0, y: 0.15, z: 0 },
          size: { x: 0.2, y: 0.2, z: 0.2 },
          radius: 0.22,
          emissionRate: 0,
          emissionBursts: 30,
          maxParticles: 64,
          particleSize: 0.1,
          lifetime: 0.8,
          speed: 4.2,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.8,
          physics: { force: { x: 0, y: -0.35, z: 0 } },
          color: '#ff9b53',
          color2: '#ffe39b',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.9,
          scaleEnd: 0.18,
        },
      ],
    }),
  },
  {
    id: 'teleportMistLite',
    label: 'Teleport Mist Lite',
    description: 'Mystic purple mist for warp and teleport transitions.',
    subcategory: 'Portal',
    props: clampParticleSystemComponentProps({
      presetId: 'teleportMistLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.8, cullDistance: 24, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#9d7bff', emissionRate: 12, burstCount: 12, radius: 0.9, speed: 0.9, lifetime: 2.6, particleSize: 0.18, opacity: 0.52 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'teleport_fog',
          shape: 'box',
          position: { x: 0, y: 0.18, z: 0 },
          size: { x: 0.6, y: 0.25, z: 0.6 },
          radius: 0.9,
          emissionRate: 12,
          emissionBursts: 12,
          maxParticles: 72,
          particleSize: 0.18,
          lifetime: 2.6,
          speed: 0.9,
          velocityMode: 'vector',
          direction: { x: 0.04, y: 1, z: 0.04 },
          spread: 0.8,
          color: '#9d7bff',
          color2: '#d5c4ff',
          alphaStart: 0.5,
          alphaEnd: 0,
          scaleStart: 0.7,
          scaleEnd: 1.4,
        },
      ],
    }),
  },
  {
    id: 'portalSwirlLite',
    label: 'Portal Swirl Lite',
    description: 'Cyan portal haze with rising motion for gateway scenes.',
    subcategory: 'Portal',
    props: clampParticleSystemComponentProps({
      presetId: 'portalSwirlLite',
      budget: { qualityTier: 'balanced', maxParticles: 88, spawnRateScale: 0.8, cullDistance: 28, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#63f0ff', emissionRate: 18, burstCount: 18, radius: 0.85, speed: 1.4, lifetime: 2.2, particleSize: 0.14, opacity: 0.7 },
      emitters: [
        {
          id: 'portal_core',
          shape: 'sphere',
          position: { x: 0, y: 0.6, z: 0 },
          size: { x: 0.3, y: 0.3, z: 0.3 },
          radius: 0.8,
          emissionRate: 18,
          emissionBursts: 18,
          maxParticles: 88,
          particleSize: 0.14,
          lifetime: 2.2,
          speed: 1.4,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 0.7,
          color: '#63f0ff',
          color2: '#d8ffff',
          alphaStart: 0.72,
          alphaEnd: 0,
          scaleStart: 0.75,
          scaleEnd: 0.22,
        },
      ],
    }),
  },
  {
    id: 'poisonPuffLite',
    label: 'Poison Puff Lite',
    description: 'Sickly green puff for poison and toxic status effects.',
    subcategory: 'Status',
    props: clampParticleSystemComponentProps({
      presetId: 'poisonPuffLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 0.7, cullDistance: 22, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#8bd96c', emissionRate: 10, burstCount: 12, radius: 0.55, speed: 0.6, lifetime: 2.8, particleSize: 0.18, opacity: 0.5 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'toxic_smoke',
          shape: 'box',
          position: { x: 0, y: 0.2, z: 0 },
          size: { x: 0.4, y: 0.2, z: 0.4 },
          radius: 0.55,
          emissionRate: 10,
          emissionBursts: 12,
          maxParticles: 64,
          particleSize: 0.18,
          lifetime: 2.8,
          speed: 0.6,
          velocityMode: 'vector',
          direction: { x: 0.08, y: 1, z: 0.08 },
          spread: 0.55,
          color: '#8bd96c',
          color2: '#d7ffb5',
          alphaStart: 0.55,
          alphaEnd: 0,
          scaleStart: 0.8,
          scaleEnd: 1.35,
        },
      ],
    }),
  },
  {
    id: 'buffAuraLite',
    label: 'Buff Aura Lite',
    description: 'Positive status aura for power-ups and temporary boosts.',
    subcategory: 'Support',
    props: clampParticleSystemComponentProps({
      presetId: 'buffAuraLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.8, cullDistance: 24, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#77ffd9', emissionRate: 14, burstCount: 14, radius: 0.85, speed: 0.95, lifetime: 2.2, particleSize: 0.12, opacity: 0.72 },
      emitters: [
        {
          id: 'buff_shell',
          shape: 'sphere',
          position: { x: 0, y: 0.7, z: 0 },
          size: { x: 0.22, y: 0.22, z: 0.22 },
          radius: 0.82,
          emissionRate: 14,
          emissionBursts: 14,
          maxParticles: 200,
          particleSize: 0.12,
          lifetime: 2.2,
          speed: 0.95,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 0.75,
          color: '#77ffd9',
          color2: '#dffff7',
          alphaStart: 0.75,
          alphaEnd: 0,
          scaleStart: 0.6,
          scaleEnd: 0.2,
        },
      ],
    }),
  },
  {
    id: 'debuffSmokeLite',
    label: 'Debuff Smoke Lite',
    description: 'Dark smoke for slowing, weakness, and negative states.',
    subcategory: 'Support',
    props: clampParticleSystemComponentProps({
      presetId: 'debuffSmokeLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 0.75, cullDistance: 22, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#8e95a3', emissionRate: 10, burstCount: 12, radius: 0.55, speed: 0.5, lifetime: 2.8, particleSize: 0.2, opacity: 0.42 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'debuff_cloud',
          shape: 'box',
          position: { x: 0, y: 0.2, z: 0 },
          size: { x: 0.45, y: 0.2, z: 0.45 },
          radius: 0.55,
          emissionRate: 10,
          emissionBursts: 12,
          maxParticles: 64,
          particleSize: 0.2,
          lifetime: 2.8,
          speed: 0.5,
          velocityMode: 'vector',
          direction: { x: -0.05, y: 0.9, z: 0.05 },
          spread: 0.6,
          color: '#8e95a3',
          color2: '#c9ced6',
          alphaStart: 0.42,
          alphaEnd: 0,
          scaleStart: 0.8,
          scaleEnd: 1.3,
        },
      ],
    }),
  },
  {
    id: 'thunderZapLite',
    label: 'Thunder Zap Lite',
    description: 'Blue lightning spark burst for electric hits and stun effects.',
    subcategory: 'Impact',
    props: clampParticleSystemComponentProps({
      presetId: 'thunderZapLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.08 },
      budget: { qualityTier: 'mini-safe', maxParticles: 56, spawnRateScale: 1, cullDistance: 18, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#67d8ff', emissionRate: 0, burstCount: 18, radius: 0.18, speed: 4.8, lifetime: 0.75, particleSize: 0.08, opacity: 0.95 },
      emitters: [
        {
          id: 'zap_core',
          shape: 'point',
          position: { x: 0, y: 0.18, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.18,
          emissionRate: 0,
          emissionBursts: 18,
          maxParticles: 56,
          particleSize: 0.08,
          lifetime: 0.75,
          speed: 4.8,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.9,
          color: '#67d8ff',
          color2: '#ffffff',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.8,
          scaleEnd: 0.15,
        },
      ],
    }),
  },
  {
    id: 'windGustLite',
    label: 'Wind Gust Lite',
    description: 'Directional streaks for strong wind and motion cues.',
    subcategory: 'Wind',
    props: clampParticleSystemComponentProps({
      presetId: 'windGustLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.8, cullDistance: 30, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#f0f7ff', emissionRate: 16, burstCount: 18, radius: 1.4, speed: 2.8, lifetime: 1.4, particleSize: 0.06, opacity: 0.5 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'wind_sheet',
          shape: 'box',
          position: { x: 0, y: 1.5, z: 0 },
          size: { x: 3.8, y: 0.4, z: 0.4 },
          radius: 1.4,
          emissionRate: 16,
          emissionBursts: 18,
          maxParticles: 200,
          particleSize: 0.06,
          lifetime: 1.4,
          speed: 2.8,
          velocityMode: 'vector',
          direction: { x: 1, y: 0, z: 0 },
          spread: 0.28,
          color: '#f0f7ff',
          color2: '#d9eaff',
          alphaStart: 0.45,
          alphaEnd: 0,
          scaleStart: 0.75,
          scaleEnd: 0.4,
        },
      ],
    }),
  },
  {
    id: 'waterSplashLite',
    label: 'Water Splash Lite',
    description: 'Splash burst for water impacts and jump landings.',
    subcategory: 'Elemental',
    props: clampParticleSystemComponentProps({
      presetId: 'waterSplashLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.08 },
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 1, cullDistance: 20, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#8fe9ff', emissionRate: 0, burstCount: 24, radius: 0.2, speed: 3.6, lifetime: 1.0, particleSize: 0.08, opacity: 0.9 },
      emitters: [
        {
          id: 'splash_core',
          shape: 'sphere',
          position: { x: 0, y: 0.1, z: 0 },
          size: { x: 0.16, y: 0.16, z: 0.16 },
          radius: 0.18,
          emissionRate: 0,
          emissionBursts: 24,
          maxParticles: 64,
          particleSize: 0.08,
          lifetime: 1,
          speed: 3.6,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.3,
          physics: { force: { x: 0, y: -0.8, z: 0 } },
          color: '#8fe9ff',
          color2: '#ffffff',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.8,
          scaleEnd: 0.18,
        },
      ],
    }),
  },
  {
    id: 'gravityWaterSplashLite',
    label: 'Gravity Water Splash Lite',
    description: 'A more physical water splash with stronger downward pull and visible droplets.',
    subcategory: 'Elemental',
    props: clampParticleSystemComponentProps({
      presetId: 'gravityWaterSplashLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.1 },
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 1, cullDistance: 20, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#9aefff', emissionRate: 0, burstCount: 30, radius: 0.26, speed: 3.8, lifetime: 1.25, particleSize: 0.11, opacity: 0.92 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'splash_core',
          shape: 'sphere',
          position: { x: 0, y: 0.12, z: 0 },
          size: { x: 0.18, y: 0.18, z: 0.18 },
          radius: 0.24,
          emissionRate: 0,
          emissionBursts: 30,
          maxParticles: 64,
          particleSize: 0.11,
          lifetime: 1.25,
          speed: 3.8,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.15,
          physics: { force: { x: 0, y: -0.55, z: 0 } },
          color: '#9aefff',
          color2: '#ffffff',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.95,
          scaleEnd: 0.2,
        },
      ],
    }),
  },
  {
    id: 'gravityStoneDebrisLite',
    label: 'Gravity Stone Debris Lite',
    description: 'Rocky impact debris with a heavier drop and rougher burst arc.',
    subcategory: 'Impact',
    props: clampParticleSystemComponentProps({
      presetId: 'gravityStoneDebrisLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.14 },
      budget: { qualityTier: 'mini-safe', maxParticles: 56, spawnRateScale: 1, cullDistance: 22, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#baa27d', emissionRate: 0, burstCount: 24, radius: 0.22, speed: 3.2, lifetime: 1.1, particleSize: 0.085, opacity: 0.95 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'stone_debris',
          shape: 'point',
          position: { x: 0, y: 0.1, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.22,
          emissionRate: 0,
          emissionBursts: 24,
          maxParticles: 56,
          particleSize: 0.085,
          lifetime: 1.1,
          speed: 3.2,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.4,
          physics: { force: { x: 0, y: -0.65, z: 0 } },
          color: '#baa27d',
          color2: '#8a7156',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.72,
          scaleEnd: 0.18,
        },
      ],
    }),
  },
  {
    id: 'gravityFireSparkLite',
    label: 'Gravity Fire Spark Lite',
    description: 'Falling ember-like sparks with a warm orange trail and gravity pull.',
    subcategory: 'Sparks',
    props: clampParticleSystemComponentProps({
      presetId: 'gravityFireSparkLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.08 },
      budget: { qualityTier: 'mini-safe', maxParticles: 56, spawnRateScale: 1, cullDistance: 20, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ffd78a', emissionRate: 0, burstCount: 22, radius: 0.16, speed: 4.6, lifetime: 0.95, particleSize: 0.075, opacity: 1 },
      render: { textureAssetId: null, blendMode: 'additive', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'fire_spark',
          shape: 'point',
          position: { x: 0, y: 0.1, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.16,
          emissionRate: 0,
          emissionBursts: 22,
          maxParticles: 56,
          particleSize: 0.075,
          lifetime: 0.95,
          speed: 4.6,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.55,
          physics: { force: { x: 0, y: -0.4, z: 0 } },
          color: '#ffd78a',
          color2: '#ff8e4f',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.7,
          scaleEnd: 0.14,
        },
      ],
    }),
  },
  {
    id: 'lavaSplashLite',
    label: 'Lava Splash Lite',
    description: 'Hot orange-red splash for lava and magma impacts.',
    subcategory: 'Elemental',
    props: clampParticleSystemComponentProps({
      presetId: 'lavaSplashLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.08 },
      budget: { qualityTier: 'mini-safe', maxParticles: 64, spawnRateScale: 1, cullDistance: 20, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#ff8a3d', emissionRate: 0, burstCount: 24, radius: 0.2, speed: 3.8, lifetime: 0.95, particleSize: 0.09, opacity: 0.98 },
      emitters: [
        {
          id: 'lava_core',
          shape: 'sphere',
          position: { x: 0, y: 0.12, z: 0 },
          size: { x: 0.16, y: 0.16, z: 0.16 },
          radius: 0.18,
          emissionRate: 0,
          emissionBursts: 24,
          maxParticles: 64,
          particleSize: 0.09,
          lifetime: 0.95,
          speed: 3.8,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.3,
          physics: { force: { x: 0, y: -0.45, z: 0 } },
          color: '#ff8a3d',
          color2: '#ffd36f',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.85,
          scaleEnd: 0.18,
        },
      ],
    }),
  },
  {
    id: 'frostMistLite',
    label: 'Frost Mist Lite',
    description: 'Cold blue mist for frozen zones and ice magic.',
    subcategory: 'Status',
    props: clampParticleSystemComponentProps({
      presetId: 'frostMistLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.75, cullDistance: 22, updateHz: 24 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#c9f0ff', emissionRate: 12, burstCount: 12, radius: 0.8, speed: 0.7, lifetime: 3.0, particleSize: 0.16, opacity: 0.42 },
      render: { textureAssetId: null, blendMode: 'alpha', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'frost_cloud',
          shape: 'box',
          position: { x: 0, y: 0.25, z: 0 },
          size: { x: 0.55, y: 0.2, z: 0.55 },
          radius: 0.8,
          emissionRate: 12,
          emissionBursts: 12,
          maxParticles: 200,
          particleSize: 0.16,
          lifetime: 3.0,
          speed: 0.7,
          velocityMode: 'vector',
          direction: { x: 0.05, y: 0.95, z: 0.05 },
          spread: 0.55,
          color: '#c9f0ff',
          color2: '#f7fdff',
          alphaStart: 0.42,
          alphaEnd: 0,
          scaleStart: 0.8,
          scaleEnd: 1.3,
        },
      ],
    }),
  },
  {
    id: 'energyTrailLite',
    label: 'Energy Trail Lite',
    description: 'Short flowing streaks for movement trails and dashes.',
    subcategory: 'Trails',
    props: clampParticleSystemComponentProps({
      presetId: 'energyTrailLite',
      budget: { qualityTier: 'mini-safe', maxParticles: 72, spawnRateScale: 0.8, cullDistance: 24, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#7feeff', emissionRate: 18, burstCount: 16, radius: 0.35, speed: 2.4, lifetime: 1.2, particleSize: 0.06, opacity: 0.85 },
      render: { textureAssetId: null, blendMode: 'additive', depthWrite: false, sortOffset: 0, renderMode: 'sprite' },
      emitters: [
        {
          id: 'trail_stream',
          shape: 'box',
          position: { x: 0, y: 0.05, z: 0 },
          size: { x: 0.2, y: 0.2, z: 0.2 },
          radius: 0.35,
          emissionRate: 18,
          emissionBursts: 16,
          maxParticles: 200,
          particleSize: 0.06,
          lifetime: 1.2,
          speed: 2.4,
          velocityMode: 'vector',
          direction: { x: 0, y: 1, z: 0 },
          spread: 0.4,
          color: '#7feeff',
          color2: '#ffffff',
          alphaStart: 0.85,
          alphaEnd: 0,
          scaleStart: 0.45,
          scaleEnd: 0.1,
        },
      ],
    }),
  },
  {
    id: 'beamHitLite',
    label: 'Beam Hit Lite',
    description: 'Quick white-blue burst for laser and beam impacts.',
    subcategory: 'Impact',
    props: clampParticleSystemComponentProps({
      presetId: 'beamHitLite',
      playback: { autoPlay: false, loop: false, startActive: false, prewarmSeconds: 0, softStopSeconds: 0.08 },
      budget: { qualityTier: 'mini-safe', maxParticles: 48, spawnRateScale: 1, cullDistance: 18, updateHz: 30 },
      exposedParams: { ...DEFAULT_PARTICLE_EXPOSED_PARAMS, color: '#e8f7ff', emissionRate: 0, burstCount: 18, radius: 0.18, speed: 4.2, lifetime: 0.7, particleSize: 0.08, opacity: 1 },
      emitters: [
        {
          id: 'beam_flash',
          shape: 'point',
          position: { x: 0, y: 0.1, z: 0 },
          size: { x: 0, y: 0, z: 0 },
          radius: 0.18,
          emissionRate: 0,
          emissionBursts: 18,
          maxParticles: 48,
          particleSize: 0.08,
          lifetime: 0.7,
          speed: 4.2,
          velocityMode: 'radial',
          direction: { x: 0, y: 1, z: 0 },
          spread: 2.6,
          color: '#e8f7ff',
          color2: '#9edcff',
          alphaStart: 1,
          alphaEnd: 0,
          scaleStart: 0.75,
          scaleEnd: 0.15,
        },
      ],
    }),
  },
]

const PRESET_MAP = new Map(PRESETS.map((entry) => [entry.id, entry]))

export function listParticlePresets(): ParticlePresetDefinition[] {
  return PRESETS.map((entry) => ({
    ...entry,
    category: resolveParticlePresetCategory(entry.id),
    subcategory: resolveParticlePresetSubcategory(entry.id),
    props: cloneParticleSystemComponentProps(entry.props),
  }))
}

export function getParticlePresetDefinition(id: string | null | undefined): ParticlePresetDefinition | null {
  const normalized = typeof id === 'string' ? id.trim() : ''
  const found = normalized ? PRESET_MAP.get(normalized) ?? null : null
  if (!found) {
    return null
  }
  return {
    ...found,
    category: resolveParticlePresetCategory(found.id),
    subcategory: resolveParticlePresetSubcategory(found.id),
    props: cloneParticleSystemComponentProps(found.props),
  }
}

export function createParticlePresetProps(id: string | null | undefined): ParticleSystemComponentProps {
  const found = getParticlePresetDefinition(id)
  return cloneParticleSystemComponentProps(found?.props ?? PRESETS[0]!.props)
}

export function listParticlePresetGroups(): ParticlePresetGroup[] {
  const grouped = new Map<ParticlePresetCategory, Map<string, ParticlePresetDefinition[]>>()
  for (const entry of listParticlePresets()) {
    const categoryBucket = grouped.get(entry.category) ?? new Map<string, ParticlePresetDefinition[]>()
    const subBucket = categoryBucket.get(entry.subcategory) ?? []
    subBucket.push(entry)
    categoryBucket.set(entry.subcategory, subBucket)
    grouped.set(entry.category, categoryBucket)
  }
  return PARTICLE_PRESET_CATEGORY_ORDER
    .map((category) => ({
      category,
      label: category,
      subgroups: Array.from(grouped.get(category)?.entries() ?? [])
        .map(([subcategory, presets]) => ({
          subcategory,
          label: subcategory,
          presets,
        }))
        .sort((a, b) => a.label.localeCompare(b.label, 'en')),
    }))
    .filter((group) => group.subgroups.length > 0)
}
