...
import { buildProceduralCityGridGroup } from './proceduralCityGrid'
import {
...
import type { ProceduralCityStyle } from './proceduralCityMaterials'
import {
  PROCEDURAL_CITY_SOLID_DEFAULT_COLOR,
  configureSolidOutlineMesh,
  getProceduralCityStyleTheme,
  getSolidOutlineMaterial,
  getWallMaterial,
  resolveProceduralCityStyle,
} from './proceduralCityMaterials'
...
// the style vocabulary and its solid default live with the shared material module;
// re-exported here because this file is the public entry point for the component
export type { ProceduralCityStyle } from './proceduralCityMaterials'
export { PROCEDURAL_CITY_SOLID_DEFAULT_COLOR } from './proceduralCityMaterials'
...
export interface ProceduralCityComponentProps {
  ...
  gridBlockDepth: number
  gridStreetWidth: number
  gridSidewalkWidth: number
  gridCurbHeight: number
  gridLotsX: number
  gridLotsZ: number
  gridFurnitureDensity: number
...
export function clampProceduralCityComponentProps(
  ...
  return {
    ...
    gridBlockWidth: clampNumber(props?.gridBlockWidth, PROCEDURAL_CITY_DEFAULT_PROPS.gridBlockWidth, 20, 500),
    gridBlockDepth: clampNumber(props?.gridBlockDepth, PROCEDURAL_CITY_DEFAULT_PROPS.gridBlockDepth, 20, 500),
    gridStreetWidth: clampNumber(props?.gridStreetWidth, PROCEDURAL_CITY_DEFAULT_PROPS.gridStreetWidth, 4, 100),
    gridSidewalkWidth: clampNumber(props?.gridSidewalkWidth, PROCEDURAL_CITY_DEFAULT_PROPS.gridSidewalkWidth, 1, 30),
    gridCurbHeight: clampNumber(props?.gridCurbHeight, PROCEDURAL_CITY_DEFAULT_PROPS.gridCurbHeight, 0, 3),
    gridLotsX: Math.trunc(clampNumber(props?.gridLotsX, PROCEDURAL_CITY_DEFAULT_PROPS.gridLotsX, 1, 20)),
    gridLotsZ: Math.trunc(clampNumber(props?.gridLotsZ, PROCEDURAL_CITY_DEFAULT_PROPS.gridLotsZ, 1, 20)),
    gridFurnitureDensity: clampNumber(props?.gridFurnitureDensity, PROCEDURAL_CITY_DEFAULT_PROPS.gridFurnitureDensity, 0, 1),
...
export function syncProceduralCityRuntimeArtifact(
  ...
  if (props.style === 'grid') {
    ...
    group = buildProceduralCityGridGroup(footprint.points, props, surfaceY)
  } else {
    const parcels = generateProceduralCityParcels(footprint, props, surfaceY)
    if (!parcels.length) {
      return
    }
    group = buildProceduralCityGroup(parcels, props.style)
  }
  const existingGroup = userData[PROCEDURAL_CITY_RUNTIME_GROUP_KEY] as Object3D | null | undefined
  if (existingGroup) {
    existingGroup.parent?.remove(existingGroup)
