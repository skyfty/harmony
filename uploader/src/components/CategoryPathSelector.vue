...
function toOption(category: ResourceCategory): CategoryOption {
  ...
  return {
    id: category.id,
    label: pathString.length ? pathString : category.name,
    pathString: pathString.length ? pathString : category.name,
    path,
  }
}
...
async function handleCreate(pathString: string): Promise<void> {
  ...
  try {
    ...
    emit('category-selected', { id: created.id, label: option.pathString })
  } catch (error) {
...
function handleChange(value: string | null): void {
  ...
  if (option) {
    emit('category-selected', { id: value, label: option.pathString })
  }
}
